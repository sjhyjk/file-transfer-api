...
class FileServiceServicer(file_pb2_grpc.FileServiceServicer):
    ...
    async def NotifyIngest(self, request: file_pb2.FileIngestEvent, context: grpc.aio.ServicerContext):
        """Go からのファイルアップロード完了通知を受け取り、RAGパイプラインを非同期駆動する"""
        try:
            ...
            logger.info(
                ...
                f"TraceID: {trace_id} | Tenant: {tenant_id} | File: {file_name} (ID: {file_id})"
            )
            ...
            if result.get("status") == "success":
                return file_pb2.FileIngestResponse(
                    status="success",
                    message=f"Successfully processed RAG pipeline via gRPC. Created {result.get('chunks_count')} chunks."
                )
            else:
                # context.abort を非同期で await 呼び出し（grpc.aio の標準仕様）
                logger.warning(f"⚠️ [gRPC Ingest] Pipeline returned failure status: {result.get('message')}")
                await context.abort(grpc.StatusCode.INTERNAL, result.get("message", "Pipeline failed"))
