...
- **Domain**: 数学的な「定義の抽象化」を意識し、ファイルエンティティとリポジトリの外部通信用（Repository/Pipeline）インターフェースをここに配置し、全レイヤーの依存の頂点として定義。
...
- **Container Registry**: ビルドされたイメージは Google Cloud の **Artifact Registry** へ自動転送（準備完了）。
...
- [x] **GitHub Actions による CI 構築** 🎉 *Done*
  - `go test` による自動ユニットテストを全プッシュ時に実行し、デグレードを未然に防止。
  - Docker マルチステージビルドの自動化により、開発環境（Docker Desktop等）に依存しないセキュアなコンテナビルドを実現。

- [ ] **Cloud Run への自動デプロイ (CD)**
  - Artifact Registry にプッシュされたイメージを Cloud Run へデプロイし、API を公開
...
├── rag_pipeline/       # RAG インジェスト基盤への拡張用 (In Progress)
